...
Public Sub RefreshSPLSeriesRanges()
    ...
    Dim nbXColumn As Long, nbYColumn As Long, obYColumn As Long
    ...
    lastObRow = SpillLastRow(ws.Range("CP6"), SPL_DATA_FIRST_ROW)
    ...
    For chartIndex = LBound(chartNames) To UBound(chartNames)
        For modelIndex = 1 To 6
            ...
            obYColumn = ws.Range("CR1").Column + _
            ...
            Set seriesItem = ws.ChartObjects(CStr(chartNames(chartIndex))). _
                             Chart.FullSeriesCollection(seriesIndex)
            ...
            Set seriesItem = ws.ChartObjects(CStr(chartNames(chartIndex))). _
                             Chart.FullSeriesCollection(seriesIndex + 1)
            If Len(modelName) > 0 Then seriesItem.Name = modelName & " OB"
            seriesItem.XValues = ws.Range( _
                ws.Cells(SPL_DATA_FIRST_ROW, ws.Range("CQ1").Column), _
                ws.Cells(lastObRow, ws.Range("CQ1").Column))
            seriesItem.Values = ws.Range( _
