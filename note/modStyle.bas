...
Private Function SetModelBandColorLong(ByVal modelName As String, ByVal bandKey As String, _
    ...
    ' Projection has no live series yet. NB/OB update immediately.
    Dim ch As Variant, s As Series, iNB As Long, iOB As Long
    iNB = (jHit - 1) * 2 + 1
    iOB = iNB + 1
    For Each ch In TargetCharts()
        On Error Resume Next
        If ch.SeriesCollection.Count >= iOB Then
            If bandKey = "NB" Or bandKey = "ALL" Then
                Set s = ch.SeriesCollection(iNB): s.Format.Line.ForeColor.RGB = clr
            End If
            If bandKey = "OB" Or bandKey = "ALL" Then
                Set s = ch.SeriesCollection(iOB): s.Format.Line.ForeColor.RGB = clr
            End If
        End If
        On Error GoTo 0
    Next ch
...
' NB/OB legend label "MODEL NB" / "MODEL OB" on LISTS!G.
...
Private Function ActiveSheetCharts() As Collection
    ...
    If ws.Name <> "SPL" And ws.Name <> "THIRD_OCTAVE" And ws.Name <> "OVERALL" Then GoTo done
...
Public Sub ApplyAxisScale(ByVal isY As Boolean, ByVal minV As String, ByVal maxV As String, _
    ...
    modRebuild.ApplyConfiguredAxes
End Sub
