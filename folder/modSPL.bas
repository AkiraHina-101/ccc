...
Public Sub RefreshSPL()
    ...
    On Error GoTo fail
    currentAction = "Open worksheet SPL"
    ...
    currentAction = "Calculate CONFIG"
    configSheet.Calculate
    currentAction = "Read named range loadCurrentList"
    ...
    currentAction = "Read named range rpmCurrentList"
    ...
    currentAction = "Calculate SPL"
    splSheet.Calculate
    currentAction = "Relink SPL chart data ranges"
    RefreshSPLSeriesRanges
    Exit Sub
fail:
    Err.Raise Err.Number, "RefreshSPL", _
...
Public Sub RefreshSPLSeriesRanges()
    ...
    currentAction = "Format SPL OB helper data"
    FormatSPLObChartData ws

    For chartIndex = LBound(chartNames) To UBound(chartNames)
        currentAction = "Open chart SPL!" & CStr(chartNames(chartIndex))
        ...
        For modelOrdinal = 1 To modelCount
            ...
            currentAction = "Relink NB series: Chart=" & _
                            CStr(chartNames(chartIndex)) & _
            ...
            currentAction = "Relink OB series: Chart=" & _
                            CStr(chartNames(chartIndex)) & _
                            "; Series=" & CStr(seriesIndex + 1) & _
                            "; Model=" & modelName
    ...
    currentAction = "Apply SPL series visibility"
    ApplySPLSeriesVisibility ws
    Exit Sub
fail:
    Err.Raise Err.Number, "RefreshSPLSeriesRanges", _
              "Action=" & currentAction & "; " & Err.Description
End Sub
