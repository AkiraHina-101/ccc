...
Public Sub RefreshSeriesLineStyles()
    ...
    currentStep = "Validate CONFIG Model list"
    Debug.Print currentStep
    modData.ValidateModelSlots

    currentStep = "Refresh SPL chart data ranges"
    Debug.Print currentStep
    modSPL.RefreshSPLSeriesRanges

    currentStep = "Refresh OVERALL chart data ranges"
    Debug.Print currentStep
    modOverall.RefreshOverallSeriesRanges

    currentStep = "Apply CONFIG colors and line styles"
    Debug.Print currentStep
    ApplySeriesLineStyles

    currentStep = "Synchronize OVERALL markers"
    Debug.Print currentStep
    modOverall.RefreshOverallMarkerStyles

    currentStep = "Refresh Model buttons and headers"
    Debug.Print currentStep
...
fail:
    ...
    Debug.Print String$(72, "=")
    MsgBox "Action: " & currentStep & vbCrLf & _
           "Failed (" & Err.Number & "): " & Err.Description, _
        ...
        For Each chartName In chartNames
            ...
            For Each seriesItem In seriesList
                ...
                If Len(modelName) > 0 Then
                    ...
                    End If
                End If
                ApplyOneLineStyle seriesItem, modelIndex, styleOffset, _
                                  updateSeriesFill
            Next seriesItem
...
Private Sub ApplyOneLineStyle(ByVal seriesItem As Series, _
    ...
    Set config = ThisWorkbook.Worksheets("CONFIG")
    If modelIndex < 1 Or modelIndex > MAX_MODEL_COUNT Then _
        Err.Raise vbObjectError + 835, "ApplyOneLineStyle", _
                  "Unsupported CONFIG Model position: " & modelIndex
    Set modelCell = config.Cells(8 + modelIndex, "A")
    modelName = CStr(modelCell.Value2)
    If Len(Trim$(modelName)) = 0 Then Exit Sub
