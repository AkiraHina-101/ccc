...
' Moves the configured Single-Band dropdown to the previous list item.
Public Sub DropdownPreviousItem()
    MoveConfiguredDropdown DropdownTargetCell(), -1
End Sub

' Moves the configured Single-Band dropdown to the next list item.
Public Sub DropdownNextItem()
    MoveConfiguredDropdown DropdownTargetCell(), 1
End Sub

' Moves the configured Multi-Band dropdown to the previous list item.
Public Sub DropdownPreviousItemJ2()
    MoveConfiguredDropdown DropdownTargetCellJ2(), -1
End Sub

' Moves the configured Multi-Band dropdown to the next list item.
Public Sub DropdownNextItemJ2()
    MoveConfiguredDropdown DropdownTargetCellJ2(), 1
End Sub

Private Function DropdownTargetCell() As Range
    ' EDIT ONLY THIS LINE when reusing the J3 buttons for another dropdown.
    Set DropdownTargetCell = ThisWorkbook.Worksheets("OVERALL-1_3 OctaveBand").Range("J3")
End Function

Private Function DropdownTargetCellJ2() As Range
    ' EDIT ONLY THIS LINE when reusing the J2 buttons for another dropdown.
    Set DropdownTargetCellJ2 = ThisWorkbook.Worksheets("OVERALL-1_3 OctaveBand").Range("J2")
End Function

Private Sub MoveConfiguredDropdown(ByVal target As Range, ByVal direction As Long)
    Dim items As Collection
    Dim currentIndex As Long, nextIndex As Long, itemIndex As Long

    On Error GoTo failed
    If target.Validation.Type <> xlValidateList Then
        Err.Raise vbObjectError + 920, "MoveConfiguredDropdown", _
                  target.Address(False, False) & " is not a dropdown list."
    End If

    Set items = DropdownItems(target)
    If items.Count = 0 Then
        Err.Raise vbObjectError + 921, "MoveConfiguredDropdown", _
                  "The dropdown list contains no values."
    End If

    For itemIndex = 1 To items.Count
        If StrComp(CStr(items(itemIndex)), CStr(target.Value2), vbTextCompare) = 0 Then
    ...
    If currentIndex = 0 Then
        ...
        End If
    Else
        nextIndex = currentIndex + Sgn(direction)
        If nextIndex < 1 Then nextIndex = items.Count
        If nextIndex > items.Count Then nextIndex = 1
    End If

    target.Value2 = items(nextIndex)
    target.Calculate
    Exit Sub

failed:
    MsgBox "Dropdown navigation failed: " & Err.Description, _
           vbExclamation, "Dropdown navigation"
End Sub

Private Function DropdownItems(ByVal target As Range) As Collection
    Dim result As New Collection, sourceRange As Range, sourceCell As Range
    Dim formulaText As String, rawItems As Variant, item As Variant
    Dim evaluated As Variant, rowIndex As Long, columnIndex As Long

    formulaText = CStr(target.Validation.Formula1)
    If Left$(formulaText, 1) = "=" Then
        On Error Resume Next
        Set sourceRange = Application.Evaluate(formulaText)
        On Error GoTo 0
        If Not sourceRange Is Nothing Then
            For Each sourceCell In sourceRange.Cells
                If Len(CStr(sourceCell.Value2)) > 0 Then result.Add sourceCell.Value2
            Next sourceCell
        Else
            evaluated = Application.Evaluate(formulaText)
            If IsArray(evaluated) Then
                For rowIndex = LBound(evaluated, 1) To UBound(evaluated, 1)
                    For columnIndex = LBound(evaluated, 2) To UBound(evaluated, 2)
                        If Len(CStr(evaluated(rowIndex, columnIndex))) > 0 Then _
                            result.Add evaluated(rowIndex, columnIndex)
                    Next columnIndex
                Next rowIndex
            ElseIf Len(CStr(evaluated)) > 0 Then
                result.Add evaluated
            End If
        End If
    Else
        rawItems = Split(formulaText, Application.International(xlListSeparator))
        For Each item In rawItems
            If Len(Trim$(CStr(item))) > 0 Then result.Add Trim$(CStr(item))
        Next item
    End If
    Set DropdownItems = result
End Function
...
Private Sub SyncSeriesDataLabels(ByVal sourceSeries As Series, _
    ...
    For labelIndex = 1 To labelCount
        CopyOneDataLabelFormat sourceSeries.DataLabels(labelIndex), _
                               targetSeries.DataLabels(labelIndex)
    Next labelIndex
    On Error GoTo 0
End Sub

' Copies label font, number display, position, fill, border and text layout.
Private Sub CopyOneDataLabelFormat(ByVal sourceLabel As Object, _
    ...
    targetLabel.Separator = sourceLabel.Separator
    targetLabel.Orientation = sourceLabel.Orientation
    targetLabel.Format.TextFrame2.MarginLeft = _
        sourceLabel.Format.TextFrame2.MarginLeft
    targetLabel.Format.TextFrame2.MarginRight = _
        sourceLabel.Format.TextFrame2.MarginRight
    targetLabel.Format.TextFrame2.MarginTop = _
        sourceLabel.Format.TextFrame2.MarginTop
    targetLabel.Format.TextFrame2.MarginBottom = _
        sourceLabel.Format.TextFrame2.MarginBottom
    targetLabel.Format.TextFrame2.TextRange.ParagraphFormat.Alignment = _
        sourceLabel.Format.TextFrame2.TextRange.ParagraphFormat.Alignment
    targetLabel.Format.TextFrame2.VerticalAnchor = _
        sourceLabel.Format.TextFrame2.VerticalAnchor
    On Error GoTo 0
End Sub
