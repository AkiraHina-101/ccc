...
' modSPLControls; this module only owns visual hierarchy, controls and chart styling.
...
Private Sub UpdateNavigationHighlight(ByVal activeViewName As String)
    ...
    Set shp = ws.Shapes("wbNavPlot")
    On Error GoTo 0
    If shp Is Nothing Then Exit Sub
    metadata = shp.AlternativeText
    If Not TryReadShapeColor(metadata, "NavBaseFill", baseFill) Then baseFill = shp.Fill.ForeColor.RGB
    If Not TryReadShapeColor(metadata, "NavBaseText", baseText) Then baseText = shp.TextFrame2.TextRange.Font.Fill.ForeColor.RGB
    shp.AlternativeText = metadata
    If StrComp(activeViewName, "SPL", vbTextCompare) = 0 Then
...
' geometry is user-owned; this routine owns only visual hierarchy.
...
Private Sub StyleConfig(ByVal ws As Worksheet)
    Dim lastSelection As Long, lastModel As Long, lastSeries As Long, lastMulti As Long
    With ws
        .Cells.Font.Name = UI_FONT_NAME
        .Cells.Font.Color = C_Ink()
        .Range("A1:Q2").UnMerge
        ...
        ' Remove the obsolete reference-only instruction block. The editable
        ' CSV header map at AA:AE remains the import source of truth.
        .Range("AA20:AE35").ClearContents
        .Range("AA20:AE35").ClearFormats
        .Range("AA1").Value = "CSV HEADER MAPPING"
        .Range("AD5").Value = "CSV Header"

        ' Compact, grouped technical-state panel. Runtime addresses stay fixed,
        ' but spacer columns make each responsibility visually distinct.
        .Range("AG47:AT1050").ClearFormats
        .Range("AJ:AJ,AP:AP,AS:AS").ColumnWidth = 2.5
        .Columns("AG").ColumnWidth = 24
        .Columns("AH:AI").ColumnWidth = 12
        .Columns("AK").ColumnWidth = 15
        .Columns("AL").ColumnWidth = 10
        .Columns("AM").ColumnWidth = 20
        .Columns("AN").ColumnWidth = 15
        .Columns("AO").ColumnWidth = 10
        .Columns("AQ").ColumnWidth = 25
        .Columns("AR").ColumnWidth = 11
        .Columns("AT").ColumnWidth = 20

        .Range("AG47:AT47").Interior.Color = C_Navy()
        .Range("AG47").Value = "TECHNICAL DASHBOARD STATE"
        .Range("AG47:AT47").Font.Color = vbWhite
        .Range("AG47:AT47").Font.Bold = True
        .Range("AG47:AT47").RowHeight = 24

        .Range("AG48:AI48").Interior.Color = RGB(228, 237, 245)
        .Range("AK48:AL48").Interior.Color = RGB(228, 237, 245)
        .Range("AM48").Interior.Color = RGB(228, 237, 245)
        .Range("AN48:AO48").Interior.Color = RGB(228, 237, 245)
        .Range("AQ48:AR48").Interior.Color = RGB(228, 237, 245)
        .Range("AT48").Interior.Color = RGB(228, 237, 245)
        .Range("AG48").Value = "SELECTION"
        .Range("AK48").Value = "MODEL"
        .Range("AM48").Value = "CHART SERIES"
        .Range("AN48").Value = "BAND"
        .Range("AQ48").Value = "LAYOUT"
        .Range("AT48").Value = "RESULTS"
        With .Range("AG48:AT48")
            .Font.Color = C_Navy()
            .Font.Bold = True
            .HorizontalAlignment = xlLeft
        End With

        .Range("AG49").Value = "Load + RPM"
        .Range("AH49").Value = "RPM"
        .Range("AI49").Value = "Load"
        .Range("AK49").Value = "Model"
        .Range("AL49").Value = "On"
        .Range("AM49").Value = "Series label"
        .Range("AN49").Value = "Band"
        .Range("AO49").Value = "On"
        .Range("AQ49").Value = "Metric"
        .Range("AR49").Value = "Value"
        .Range("AT49").Value = "Multi-RPM loads"
        With .Range("AG49:AI49,AK49:AO49,AQ49:AR49,AT49")
            .Interior.Color = RGB(238, 243, 247)
            .Font.Color = C_Navy()
            .Font.Bold = True
            .HorizontalAlignment = xlCenter
        ...
        lastSelection = Application.Max(50, .Cells(.Rows.Count, CFG_RPM_LABEL_COL).End(xlUp).Row, _
                                        .Cells(.Rows.Count, CFG_RPM_VALUE_COL).End(xlUp).Row, _
                                        .Cells(.Rows.Count, CFG_LOAD_VALUE_COL).End(xlUp).Row)
        lastModel = Application.Max(50, .Cells(.Rows.Count, CFG_MODEL_NAME_COL).End(xlUp).Row, _
                                    .Cells(.Rows.Count, CFG_MODEL_ENABLED_COL).End(xlUp).Row)
        lastSeries = Application.Max(50, .Cells(.Rows.Count, CFG_SERIES_LABEL_COL).End(xlUp).Row)
        lastMulti = Application.Max(50, .Cells(.Rows.Count, CFG_MULTI_LOAD_COL).End(xlUp).Row)

        With .Range("AG50:AI" & lastSelection)
            .Interior.Color = vbWhite
            .Borders(xlInsideHorizontal).Color = RGB(232, 237, 242)
        End With
        With .Range("AK50:AL" & lastModel)
            .Interior.Color = vbWhite
            .Borders(xlInsideHorizontal).Color = RGB(232, 237, 242)
        End With
        .Range("AL50:AL" & lastModel).Interior.Color = C_Edit()
        With .Range("AM50:AM" & lastSeries)
            .Interior.Color = RGB(248, 250, 252)
            .Borders(xlInsideHorizontal).Color = RGB(232, 237, 242)
        End With
        .Range("AN50:AO51").Interior.Color = vbWhite
        .Range("AO50:AO51").Interior.Color = C_Edit()
        .Range("AQ50:AR53").Interior.Color = RGB(248, 250, 252)
        .Range("AT50:AT" & lastMulti).Interior.Color = vbWhite
        .Range("AG50:AT" & _
               Application.Max(lastSelection, lastModel, lastSeries, lastMulti, 53)).Font.Size = 10
        .Range("AH50:AI" & lastSelection).HorizontalAlignment = xlCenter
        .Range("AL50:AL" & lastModel).HorizontalAlignment = xlCenter
        .Range("AO50:AO51").HorizontalAlignment = xlCenter
        .Range("AR50:AR53").HorizontalAlignment = xlCenter
        .Range("AG54:AT1050").RowHeight = 15
    End With
    ...
    With ws
        ...
        If .Range("H1").Comment Is Nothing Then
            .Range("H1").AddComment _
                "USER CALCULATIONS: add a unique header in row 1 and a template formula in row 2."
        End If
        On Error GoTo 0
    End With
