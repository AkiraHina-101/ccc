...
proc ::nc::session::_table_headers_for_rows {tab rows} {
    set header [_default_table_header $tab]
    foreach row $rows {
...
proc ::nc::session::save_table_session {rows_by_tab {dir ""}} {
    set root [_active_or_dir $dir]
    ...
    foreach tab [_table_tabs] {
        set rows {}
        if {[dict exists $rows_by_tab $tab]} { set rows [dict get $rows_by_tab $tab] }
        set header [_table_headers_for_rows $tab $rows]
        set data {}
        foreach row $rows {
            set values {}
            foreach key $header {
