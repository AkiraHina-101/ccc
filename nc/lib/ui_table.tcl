...
proc ::nc::ui_table::_material_id_for_label {label} {
    ...
    if {[llength $ids] > 1} {
        ...
        _set_status "Multiple materials named '$label' (mat_id [join $ids {, }]) - using mat_id [lindex $ids 0]. Rename duplicates to avoid ambiguity." warn
    }
    return [lindex $ids 0]
}
...
proc ::nc::ui_table::_dropdown_values_for_key {key current} {
    ...
    switch -- $key {
        ...
        mat_user_name {
            # Source of truth is the Materials tab's mat_user_name field
            # only (no fallback to material_label/mat_name), plus the
            # pasted _label_bank above.
            if {[info exists _tab_rows(materials)]} {
...
proc ::nc::ui_table::_set_row_value {tab row key value {session_dirty 1}} {
    ...
    switch -- $key {
        ...
        mat_user_name {
            ...
            # Only re-derive mat_id from the typed label on general/component
            # rows, where mat_user_name means "pick an existing material by
            # name". On the materials tab itself, mat_user_name is the row's
            # OWN label being renamed - looking it up here could match a
            # different material row (e.g. a duplicate/renamed-to label) and
            # steal that row's mat_id onto this one.
            if {$tab ne "materials"} {
                set mid [_material_id_for_label $value]
                if {$mid ne ""} {
...
proc ::nc::ui_table::_build_menubar {root} {
    ...
    $mf.selectionMenu.menu add command -label "Invert Highlight" -accelerator "Shift+I" -command {::nc::ui_table::_on_invert_highlight}
...
proc ::nc::ui_table::_build_focusbar {parent} {
    ...
    _add_button $f inverthighlight "Invert" {::nc::ui_table::_on_invert_highlight} info
...
proc ::nc::ui_table::_build_table_frame {root} {
    ...
    bind $_tbl <Shift-I> {::nc::ui_table::_on_invert_highlight; break}
...
proc ::nc::ui_table::_update_toolbar_for_tab {} {
    ...
    foreach f [list $_assign_frame $_io_frame $_label_frame $_edit_frame $_review_frame $_orientation_frame $_view_frame $_library_frame $_prop_view_frame $_pbush_frame] {
    ...
    if {$_tab eq "library"} {
        ...
        if {$_library_frame ne "" && [winfo exists $_library_frame]} {
            catch {pack $_library_frame -side left -padx {0 6} -pady {2 4}}
        }
    ...
    if {$_show_review_toolbar && $_orientation_frame ne "" && [winfo exists $_orientation_frame]} {
        catch {pack $_orientation_frame -side left -padx {0 6} -pady {2 4}}
    }
...
proc ::nc::ui_table::_show_context_menu {X Y} {
    ...
    $_context_menu add command -label "Invert Highlight" -accelerator "Shift+I" -command {::nc::ui_table::_on_invert_highlight}
...
proc ::nc::ui_table::_on_highlight_comp {} {
    ...
    set _highlight_active_gray_ids $other_ids
    _set_status "Highlighted [llength $comp_ids] component(s); [llength $other_ids] other(s) greyed out." ok
}

# Swaps which components are grey: whatever is currently grey (from the last
# Highlight Comp / Invert Highlight call) goes back to normal, and whatever
# was normal becomes grey instead. Same no-isolate, no-rotate mechanism as
# Highlight Comp - just the two sets swapped.
proc ::nc::ui_table::_on_invert_highlight {} {
    variable _highlight_active_gray_ids
    if {![_color_api_available] || [llength [info commands *autocolorwithmark]] == 0} {
        _set_status "the CAE tool color commands are not available in this session." warn
        return
    ...
    set all_ids [_all_known_comp_ids]
    if {[llength $all_ids] == 0} {
        _set_status "No known components to invert." warn
        return
    }
    set old_gray $_highlight_active_gray_ids
    set new_gray {}
    foreach cid $all_ids {
        if {[lsearch -exact $old_gray $cid] < 0} { lappend new_gray $cid }
    }
    set rc [catch {
        catch {*startnotehistorystate {ColorMod {Components}}}
        if {[llength $old_gray] > 0} {
            catch {*clearmark components 1}
            *createmark components 1 "by id" {*}$old_gray
            *autocolorwithmark components 1
        }
        if {[llength $new_gray] > 0} {
            catch {*clearmark components 1}
            *createmark components 1 "by id" {*}$new_gray
            *colormark components 1 $color_id
        }
        catch {*endnotehistorystate {ColorMod {Components}}}
        catch {*window 0 0 0 0 0}
    ...
    if {$rc} {
        _set_status "Invert Highlight failed: $err" error
        return
    }
    set _highlight_active_gray_ids $new_gray
    _set_status "Inverted highlight: [llength $new_gray] component(s) now greyed out, [llength $old_gray] restored." ok
}
...
# Clears any transparency applied by Find Comp AND any grey-out applied by
# Highlight Comp, restoring every known component to normal opaque display
# with its own auto-assigned color (*autocolorwithmark) - a single "back to
# normal" action for both of the mutually-related highlight features.
proc ::nc::ui_table::_on_reset_transparency {} {
    ...
    _set_status "Reset [llength $all_ids] component(s) (transparency + color)." ok
}
