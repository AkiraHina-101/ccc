...
# The "other components" universe Find Comp should dim - restricted to
# whatever the table is CURRENTLY showing (after Search/Worklist/Property
# View filtering), not every component ever loaded. So filtering the table
# down to a handful of rows and hitting Find Comp on one of them only dims
# the other rows still on screen, leaving components the filter already
# hid alone. Falls back to the full known universe when the active tab
# isn't Component/General (Properties/Materials rows don't map to a
# single filtered component list of their own).
proc ::nc::ui_table::_displayed_comp_ids_universe {} {
    variable _tab
    if {$_tab in {component general}} {
        set out {}
        foreach row [_rows_for_display] {
            set cid [_dict_get $row comp_id]
            if {$cid ne "" && [string is integer -strict $cid] && $cid > 0 && [lsearch -exact $out $cid] < 0} {
                lappend out $cid
            }
...
proc ::nc::ui_table::_apply_findcomp_action {target other} {
    ...
    set rc [catch {
        ...
        *isolateonlyentitybymark 2 1 2
        ...
        # part of "other" back when a different component was targeted).
        catch {*clearmark components 1}
...
# normal" action for both of the mutually-related highlight features. Scoped
# to whatever the table is currently displaying (same universe Find
# Comp/Highlight dim against), not every component ever loaded - components
# a filter is currently hiding are left untouched.
