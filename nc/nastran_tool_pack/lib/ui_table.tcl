...
# set, then optionally fit the view (*window 0 0 0 0 0) - no rotate, no isolate.
...
# Clears only the components this tool actually changed: transparency from the
# last Find Comp, and grey color from the last Highlight Comp. Do not build a
# fresh "all/displayed" mark here: reset must not alter visibility or global FE
# display style when there is no recorded work to undo.
proc ::nc::ui_table::_on_reset_transparency {} {
    ...
    if {[llength $trans_ids] == 0 && [llength $color_ids] == 0} {
        _set_status "Nothing to reset - no Find/Highlight changes are recorded." warn
        return
    }
    ...
    _set_status "Reset transparency/color for recorded Find/Highlight component(s)." ok
}
