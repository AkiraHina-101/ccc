...
# scan.tcl  —  ::nc::scan
#
# Pure read module: scan the CAE tool model -> list-of-dicts for ui_table.tcl.
...
proc ::nc::scan::ensure_template {} {
    ...
    set candidate ""
    if {[llength [info commands ::nc::config::resolve_template]] > 0} {
        set candidate [::nc::config::resolve_template]
    }
...
# -----------------------------------------------------------------------------
# Private: _fix_mojibake
# the CAE tool's hm_getvalue/hm_getentityvalue return raw CP932 (Shift-JIS) bytes
# for non-ASCII text (e.g. Japanese component/note names) instead of proper
# Tcl Unicode strings. Tcl then treats each byte as a Latin-1 codepoint,
# producing garbage like "|Þ" for text that should read as Japanese. Reverse
# that mis-interpretation: round-trip through iso8859-1 to recover the raw
# bytes, then decode them as CP932.
#
# Guard: only do this when every character in the string already has a code
# point <= 255 - i.e. it can only ever be a mis-decoded raw byte string, never
# genuine Unicode text (real non-Latin-1 text, like Vietnamese diacritics or
# correctly-returned Japanese, uses code points > 255 and must be left alone).
# Without this guard, `encoding convertto iso8859-1` can silently replace
# unmappable characters with "?" on older Tcl, corrupting already-correct text.
# -----------------------------------------------------------------------------

proc ::nc::scan::_fix_mojibake {value} {
    if {$value eq ""} { return $value }
    foreach c [split $value ""] {
        if {[scan $c %c cp] && $cp > 255} { return $value }
    }
    set fixed $value
    if {[catch {set fixed [encoding convertfrom cp932 [encoding convertto iso8859-1 $value]]}]} {
        return $value
    }
    return $fixed
}
...
proc ::nc::scan::_get_name {etype id} {
    foreach script [list \
        ...
        if {![catch $script value] && [string trim $value] ne ""} {
            return [_fix_mojibake [string trim $value]]
        }
    }
    return ""
}
...
#
# _field_aliases: the CAE tool 2022 does not expose every card field under its
...
proc ::nc::scan::_get_card_field {etype id field} {
    ...
    if {[info exists _field_aliases($etype,$field)]} {
        set alias_name $_field_aliases($etype,$field)
        set names [linsert $names 0 $alias_name]
    }
    set first_value ""
    foreach name $names {
        foreach script [list \
            [list hm_getvalue $etype id=$id dataname=$name] \
            [list hm_getentityvalue $etype +$id $name 0 -byid] \
            [list hm_getentityvalue $etype $id $name 0]] {
            if {![catch $script value]} {
                set value [_fix_mojibake [string trim $value]]
                # A numeric-attribute alias (e.g. MAT1 GE -> dataname 7) can
                # come back as the literal dataname token itself when the
                # field genuinely isn't set on this card - the CAE tool
                # echoes the query argument instead of erroring. That's not
                # real data, so treat it as absent rather than accepting it
                # as the field's value (was showing "7" for unset GE).
                if {$name eq $alias_name && $value eq $name} { continue }
                if {$value ne "" && $value ne "<None>"} {
                    if {$first_value eq ""} { set first_value $value }
...
proc ::nc::scan::_read_prop {pid} {
    ...
    foreach field {T NSM Z1 Z2} {
        dict set data $field [_get_card_field props $pid $field]
    }
...
proc ::nc::scan::_read_mat {mid} {
    ...
    foreach field {E G NU RHO A TREF GE ST SC SS} {
        dict set data $field [_get_card_field mats $mid $field]
    }
...
proc ::nc::scan::_build_row {cid} {
    ...
    foreach field $prop_fields { set prop_values($field) "" }
    foreach field $mat_fields { set mat_values($field) "" }
    ...
    if {$prop_id > 0} {
        ...
        foreach field $prop_fields {
            if {[dict exists $prop_data $field]} {
                set prop_values($field) [dict get $prop_data $field]
            }
        }
        ...
        if {$mat_id > 0} {
            ...
            foreach field $mat_fields {
                if {[dict exists $mat_data $field]} {
                    set mat_values($field) [dict get $mat_data $field]
                }
            }
        }
    }
    ...
    set row [dict create \
        ...
        elem_count -1]
    foreach field $prop_fields {
        dict set row $field $prop_values($field)
    }
    foreach field $mat_fields {
        dict set row $field $mat_values($field)
    }
    return $row
}
