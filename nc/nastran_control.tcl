...
#
# Usage from the CAE tool:
#   *evaltclscript("<path-to-this-folder>/nastran_control.tcl", 0)
#   (or use the "NC Launch" button in nastran_control.mac — path is auto-filled)
#
...
proc ::nc::app::_session_internal_subfolder_names {} {
    return {edits cache thumb_cache Component_Images library}
}
...
proc ::nc::app::_startup_rows_for_session {} {
    ...
    if {[catch {set result [::nc::session::load_table_session $dir]} loaderr]} {
        # Post-xlsx-migration this can fail when the session is xlsx-format
        # but this machine lacks Python/openpyxl. Log the reason so the
        # user isn't left staring at a scan-only view with no clue why
        # their saved edits didn't come back.
        catch {::nc::mutations::log_add "Session cache could not be loaded: $loaderr"}
        return $empty
    }
...
proc ::nc::app::_sync_label_columns_in_rows_by_tab {rows_by_tab} {
    ...
    #
    # Deliberately does NOT default comp_user_name/mat_user_name to the
    # the CAE tool name (hm_comp_name/mat_name) when empty - that was tried and
    # reverted because it made "COMP. Name" indistinguishable from "HM Comp.
    # Name" (same for MAT. Type vs the HM material name). HM Comp./MAT name
    # is populated only by a Reload/scan from the CAE tool; the user-facing
    # name/type columns are left blank until the user fills them in.
    # NOTE: prop_user_name is deliberately NOT paired with prop_name here.
    # prop_name is the HM-scanned property name (scan-owned, like
    # hm_comp_name), not a user alias column - pairing them both filled the
    # user's "Property Label" with the HM name AND let a user label overwrite
    # the scanned prop_name. Same recurring bug class as comp_user_name
    # (docs/LESSONS_AND_PITFALLS.md #8). Only true user<->user alias pairs
    # (comp_user_name<->label, mat_user_name<->material_label) are synced.
...
proc ::nc::app::_sync_label_pair {row user_key alt_key} {
    # user_key (e.g. comp_user_name) is the field users/CSV-editors actually
    # change, so it wins when both are populated and differ. alt_key only
    # fills user_key when user_key is empty (e.g. legacy sessions that only
    # ever had the alt_key column populated).
    set uv ""; set av ""
...
proc ::nc::app::_decorate_component_rows {comp_rows} {
    ...
    foreach row $comp_rows {
        ...
        # HM Comp. Name is the only field a scan ever writes - it always
        # reflects the raw the CAE tool name. COMP. Name (comp_user_name) is
        # user-facing and must NOT default to it (that made the two
        # indistinguishable); only carry over a label the user already set.
        ...
        # Property Label (prop_user_name) is user-entered like COMP. Name -
        # never seed it from the HM property name (same recurring bug class
        # as comp_user_name); keep whatever the row already carries.
        if {![dict exists $row prop_user_name]} { dict set row prop_user_name "" }
        set material_label [expr {[dict exists $row material_label] ? [dict get $row material_label] : ""}]
        dict set row mat_user_name $material_label
...
proc ::nc::app::capture_component_images {dir comp_ids {progress_cb ""} {cancel_cb ""}} {
    ...
    if {!$have_hwi && [llength [info commands *jpegfilenamed]] == 0} {
        error "the CAE tool graphics capture API is not available."
    }
...
proc ::nc::app::_add_preview_material_fields {row} {
    ...
    # MAT. Type (mat_user_name) is user-facing and must not default to the
    # HM material name - this proc runs on every derived material (not just
    # the synthetic preview rows below), so defaulting here would make MAT.
    # Type indistinguishable from the HM-sourced Material name.
    if {![dict exists $row mat_user_name]} { dict set row mat_user_name "" }
