...
from app.ui.workers import BjobsPoller, SSHCommandWorker
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self._preview_sh_tab = PreviewTab()
        self._preview_dat_tab = PreviewTab()
        self._queue_tab = QueueTab()
        self.right_dock = RightDock(
            self._preview_sh_tab, self._preview_dat_tab,
    ...
    def _refresh_queue(self):
        """Trigger an immediate bjobs poll. Rows arrive via the poller signal."""
    ...
    def _sync_job_status_from_queue(self, rows):
        ...
        for entity, base in self._iter_submitted_entities():
            ...
            if row:
                ...
                if jid_s in seen and old_state not in ('Done', 'Error', 'Fail'):
                    entity['status'] = 'Done'
                    changed = True
                    notifications.append((
                        f'Job {jid_s} done',
    ...
    def _on_ssh_command_requested(self, command_def):
        ...
        if '{selected_jobid}' in command:
            jobid = self._queue_tab.selected_jobid()
            if not jobid:
                ...
                return
            # Per-jobid lock: prevent re-issuing the same {selected_jobid}
            # command (e.g. bkill 12345) while a prior instance for the same
            # jobid is still in flight. Other buttons stay free.
        ...
        self.right_dock.set_active_tab("terminal")
    ...
    def _on_preview_file(self, win_folder, filename):
        self._preview_dat_tab.load_file(win_folder, filename)
        self.right_dock.set_active_tab("preview_dat")

    def _on_preview_heredoc(self, text):
        self._preview_sh_tab.show_heredoc(text)
        self.right_dock.set_active_tab("preview_sh")
