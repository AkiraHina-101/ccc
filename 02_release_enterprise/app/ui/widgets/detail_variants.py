...
class _BaseJobDetail(QWidget):
    ...
    def set_status(self, status: str):
        ...
        if status == 'Running':
            ...
            self.status_banner_lbl.setText("⚠  Error — see terminal for details")
    ...
    def _build_row_submit_button(self, item):
        ...
        btn = _RowSubmitButton("…" if running else ("✓" if done else ""))
        btn.setMinimumSize(24, 24)
        btn.setMaximumSize(28, 28)
        btn.setFocusPolicy(Qt.NoFocus)
        btn.setEnabled(status in ('Pending', 'Error'))
        btn.setToolTip("Running" if running else ("Done" if done else "Submit"))
        btn.setCursor(Qt.PointingHandCursor)
        set_qt_property(
            btn,
            row_status='running' if running else ('done' if done else 'ready'),
        )
        return btn
...
class SingleJobDetail(_BaseJobDetail):
    ...
    def _build_gather_section(self):
        ...
        self.gather_source_edit.setPlaceholderText(
            "Folder to search for INCLUDE files (recursive)")
        ...
        self.gather_validate_btn.setToolTip(
            "Parse INCLUDE in the selected .dat and check which files are "
            "already in the submit folder vs gatherable from source")
        row.addWidget(self.gather_validate_btn)
        self.gather_copy_btn = QPushButton("📋 Copy missing")
        self.gather_copy_btn.setEnabled(False)
        self.gather_copy_btn.setToolTip("Copy gatherable files into the submit folder")
        row.addWidget(self.gather_copy_btn)
        self.body_layout.addLayout(row)

        self.gather_status_lbl = QLabel("")
        self.gather_status_lbl.setObjectName("hintLabel")
        self.body_layout.addWidget(self.gather_status_lbl)

        # Per-file list. Hidden until first validate to keep the panel compact.
        self.gather_list = QListWidget()
        self.gather_list.setMaximumHeight(140)
        self.gather_list.setVisible(False)
        self.body_layout.addWidget(self.gather_list)
        ...
        self.gather_copy_btn.clicked.connect(self._on_gather_copy_clicked)
    ...
    def _on_gather_validate_clicked(self):
        ...
        if not dat_path:
            self.gather_status_lbl.setText(
                "Select a folder and .dat first — nothing to scan.")
        ...
        self._render_gather_report(report, source)

    def _render_gather_report(self, report, source):
        required = report['required']
        status = report['status']
        source_paths = report.get('source_paths', {})
        missing_in_source = report.get('missing_in_source', [])
        userfiles = report.get('userfiles', [])
        userfile_status = report.get('userfile_status', {})
        ...
        if total == 0 and not userfiles:
            self.gather_status_lbl.setText(
                "No INCLUDE or ASSIGN statements found in the selected .dat.")
        else:
            parts = [f"{total} required", f"{ok_count} in submit folder"]
            if gatherable:
                parts.append(f"{len(gatherable)} gatherable from source")
            if missing_in_source:
                parts.append(f"{len(missing_in_source)} NOT FOUND anywhere")
            if missing and not source.strip() and not gatherable:
                parts.append("(pick a source folder to locate missing)")
            if userfile_mismatches:
                # Surface this loudly — wrong USERFILE name silently overwrites
                # another job's output CSV at runtime.
                parts.append(f"⚠ {len(userfile_mismatches)} USERFILE name mismatch")
            self.gather_status_lbl.setText(" · ".join(parts))

        self.gather_list.clear()
        for name in required:
            st = status.get(name, 'missing')
            if st == 'ok':
                prefix = "✓"
                detail = "in submit folder"
            elif source_paths.get(name):
                prefix = "→"
                detail = f"from {source_paths[name]}"
            else:
                prefix = "✗"
                detail = "not found in source"
            self.gather_list.addItem(f"{prefix}  {name}  —  {detail}")
        # USERFILE entries: name-sanity only, no copy involved.
        for name in userfiles:
            ust = userfile_status.get(name, 'mismatch')
            if ust == 'ok':
                self.gather_list.addItem(f"○  {name}  —  output (USERFILE), name OK")
            else:
                self.gather_list.addItem(
                    f"⚠  {name}  —  output (USERFILE), name does NOT match .dat stem")
        self.gather_list.setVisible(bool(required) or bool(userfiles))

        self.gather_copy_btn.setEnabled(bool(gatherable))
    ...
    def _on_gather_copy_clicked(self):
        ...
        # Re-validate so the list reflects the new state.
        self._on_gather_validate_clicked()
        if failed:
...
class FolderGroupDetail(_BaseJobDetail):
    ...
    def _refresh_files_table(self):
        ...
        files = self._data.get('files', [])
        if ext.startswith('.'):
            shown = [fi for fi in files if fi['name'].lower().endswith(ext)]
        else:
            shown = list(files)
        ...
        for fi in shown:
            ...
            status_item = QTableWidgetItem(fi.get('status', 'Pending'))
...
class MultiFolderDetail(_BaseJobDetail):
    ...
    def _refresh_rows_table(self):
        ...
        for rd in rows:
            ...
            sl.addWidget(dot)
            status_text = QLabel(rd.get('status', 'Pending'))
