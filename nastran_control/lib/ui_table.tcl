...
# Checks that the (unverified on this install) transparency API is present
# before we try to use it, so a wrong/missing command fails loud with a clear
# message instead of a raw Tcl error or a silently broken display.
proc ::nc::ui_table::_transparency_api_available {} {
    foreach cmd {*settransparency *transparencymark} {
        if {[llength [info commands $cmd]] == 0} { return 0 }
    }
    return 1
}
