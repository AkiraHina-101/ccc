...
proc ::nc::ui_table::_dropdown_values_for_key {key current} {
    ...
    switch -- $key {
        ...
        mat_user_name {
            # Source of truth is the Materials tab only (plus the pasted
            # _label_bank above) - _mat_rows (raw scan cache) and $_rows
            # (whichever tab happens to be open) were redundant/stale
            # duplicates of the same names and could drift from what the
            # Materials tab actually shows.
            if {[info exists _tab_rows(materials)]} {
...
proc ::nc::ui_table::_sync_material_label_across_rows {mat_id new_label} {
    variable _tab_rows
    if {$mat_id eq ""} { return }
    foreach tab {general component} {
        if {![info exists _tab_rows($tab)]} continue
...
proc ::nc::ui_table::_save_session_to_dir {dir {status 1}} {
    ...
    set result [::nc::session::save_table_session [_rows_by_tab_snapshot] $dir $header_order_by_tab]
...
proc ::nc::ui_table::_stage_cell_value {r c key new_val old_val} {
    ...
    if {$_tab in {general component}} {
        ...
        _replace_row $_tab $row_key $row
        if {$_tab eq "materials" && $key eq "mat_user_name"} {
            _sync_material_label_across_rows [_dict_get $row mat_id] [_dict_get $row mat_user_name]
        }
    }
    _populate_current
...
# Reloads the current tab's own edits/*.csv file exactly as-is, replacing the
# tab's rows outright with no id matching, merging, or preview step.
proc ::nc::ui_table::_reload_tab_from_own_file {tab path} {
    if {![file exists $path]} {
        _set_status "No saved file yet for [_tab_label $tab] tab ($path)." warn
        return
    }
    set rows {}
    set rc [catch {
...
proc ::nc::ui_table::_on_import {} {
    variable _tab
    set own_files {}
    catch {set own_files [::nc::session::table_session_files]}
    if {[dict exists $own_files $_tab] && [file exists [dict get $own_files $_tab]]} {
        set own_path [dict get $own_files $_tab]
        set choice [_ask_import_source_choice $_tab $own_path]
        if {$choice eq ""} { return }
        if {$choice eq "own"} {
            _reload_tab_from_own_file $_tab $own_path
            return
        }
    }
    set path ""
