...
# Default column set — used only when settings didn't supply queue_columns.
# The runtime column list lives in `self._columns` and is populated by
# set_columns() from persisted settings. jobid + state are always kept.
DEFAULT_COLUMNS = [
...
class QueueTab(QWidget):
    ...
    selection_changed = Signal(str)  # emits current jobid ('' when nothing selected)
    command_buttons_rebuilt = Signal()  # fires after _rebuild_command_buttons (so listeners can re-apply state)

    def __init__(self, parent=None):
        super().__init__(parent)
        self._theme_mode = 'dark'
        self._rows = []
        # Per-jobid dict of extra fields harvested from `bjobs -l JOBID`.
        # Merged onto rows at render time so an incoming short-format poll
        # doesn't wipe details the user already fetched.
        self._row_details = {}
        ...
        self._columns = [dict(c) for c in DEFAULT_COLUMNS]
        self._init_ui()
    ...
    def _init_ui(self):
        ...
        # Default OFF — polling every 5s across a Paramiko SSH round-trip is
        # noisy and the user typically only cares about status right after
        # submitting. Let them opt in explicitly; Refresh button gives on-demand.
        self.autopoll_btn = QPushButton('Auto: off')
        self.autopoll_btn.setObjectName('queueAutopollBtn')
        self.autopoll_btn.setCheckable(True)
        self.autopoll_btn.setChecked(False)
        ...
        # Tree — columns get set up by _apply_columns() below.
        ...
        # Last visible column always stretches so the header + row cells fill
        # the panel width edge-to-edge. Narrower dock → last column may get
        # clipped (or user scrolls horizontally) but there's never trailing
        # empty space, per user request.
    ...
    def _enabled_columns(self):
        """Return the list of columns to render. Always-on keys survive even
        if the user unticked them in settings (they have no other way to
        select a row).
        """
        cols = [c for c in self._columns if c.get('enabled') or c.get('key') in _ALWAYS_ON_KEYS]
        return cols or [dict(c) for c in DEFAULT_COLUMNS if c['key'] in _ALWAYS_ON_KEYS]

    def _apply_columns(self):
        cols = self._enabled_columns()
        self.tree.setColumnCount(len(cols))
        self.tree.setHeaderLabels([c.get('label') or c.get('key') for c in cols])
        header_view = self.tree.header()
        for i, c in enumerate(cols):
            header_view.resizeSection(i, int(c.get('width') or 120))

    def set_columns(self, columns):
        """Reconfigure which columns show. `columns` is the list from settings.
        Called from main_window after load and after Settings dialog closes.
        """
    ...
    def merge_row_details(self, jobid, extras):
        """Merge extra fields (from `bjobs -l JOBID`) into the row cache.

        Called from main_window when SSHCommandWorker returns parsed
        long-format rows. Renders immediately so the user sees the new
        columns fill in without waiting for the next poll.
        """
        if not jobid or not isinstance(extras, dict):
            return
        cur = self._row_details.get(str(jobid), {})
        cur.update({k: v for k, v in extras.items() if v})
        self._row_details[str(jobid)] = cur
        self._render()
    ...
    def _rebuild_command_buttons(self):
        ...
        self.command_buttons_rebuilt.emit()
    ...
    def _render(self):
        prev_selected = self.selected_jobid()
        self.tree.clear()
        ...
        for row in self._rows:
            jobid = str(row.get('jobid', row.get('id', '')))
            if jobid in self._dismissed_jobids:
                continue
            # Overlay any details previously fetched via bjobs -l JOBID.
            merged = dict(row)
            details = self._row_details.get(jobid)
            if details:
                for k, v in details.items():
                    # Don't overwrite known-good short-format fields with a
                    # possibly stale long-format value; only fill blanks.
                    if not merged.get(k):
                        merged[k] = v
            state = merged.get('state') or merged.get('status') or 'Pending'
            values = []
            for c in cols:
                key = c['key']
                if key == 'state':
                    ...
                    values.append(str(merged.get('summary') or merged.get('name') or merged.get('job_name') or ''))
                else:
                    values.append(str(merged.get(key, '')))
            item = QTreeWidgetItem(values)
            color = QColor(colors.get(state, colors['Pending']))
            for col_idx in range(len(cols)):
                item.setForeground(col_idx, color)
            self.tree.addTopLevelItem(item)
            if jobid and jobid == prev_selected:
                restore_item = item
    ...
    def _on_selection_changed(self):
        self.selection_changed.emit(self.selected_jobid())
