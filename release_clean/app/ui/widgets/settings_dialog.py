...
class SettingsDialog(QDialog):
    ...
    def _build_connection_tab(self):
        ...
        self.ssh_button_up_btn = QPushButton('▲')
        self.ssh_button_down_btn = QPushButton('▼')
        self.ssh_button_up_btn.setToolTip('Move selected row up')
        self.ssh_button_down_btn.setToolTip('Move selected row down')
        self.ssh_button_up_btn.setFixedSize(32, 26)
        self.ssh_button_down_btn.setFixedSize(32, 26)
        self.ssh_button_add_btn.setFixedHeight(26)
        self.ssh_button_del_btn.setFixedHeight(26)
        ssh_header.addWidget(self.ssh_button_up_btn)
        ssh_header.addWidget(self.ssh_button_down_btn)
        ...
        # Poller command for the Queue tab table. Distinct from SSH buttons:
        # this one runs every ~5s. Common variants: 'bjobs -a -w -u {user}'
        # (only my jobs) or 'bjobs -u all -q nast16m' (whole queue).
        poll_row = QHBoxLayout()
        poll_lbl = QLabel('Queue poll command:')
        poll_lbl.setFixedWidth(140)
        poll_row.addWidget(poll_lbl)
        self.bjobs_command_input = QLineEdit(
            self._settings.get('bjobs_command') or 'bjobs -u {user}')
        self.bjobs_command_input.installEventFilter(self)
        poll_row.addWidget(self.bjobs_command_input, stretch=1)
        lay.addLayout(poll_row)
        self._build_queue_columns_section(lay)
        lay.addStretch()
        return tab

    def _build_queue_columns_section(self, parent_layout):
        """Checkbox list of Queue columns. Core columns (jobid, state) are
        forced on; extras (bjobs -l enrichments) start off.
        """
        parent_layout.addSpacing(8)
        header = QLabel('Queue columns (bjobs -l detail is fetched on row click):')
        header.setProperty('role', 'dim')
        parent_layout.addWidget(header)

        detail_row = QHBoxLayout()
        self.queue_detail_on_select_cb = QCheckBox(
            'Auto-fetch `bjobs -l JOBID` when I click a queue row')
        self.queue_detail_on_select_cb.setChecked(
            bool(self._settings.get('queue_detail_on_select', True)))
        detail_row.addWidget(self.queue_detail_on_select_cb)
        detail_row.addStretch()
        parent_layout.addLayout(detail_row)

        self.queue_columns_table = QTableWidget()
        self.queue_columns_table.setColumnCount(2)
        self.queue_columns_table.setHorizontalHeaderLabels(['Show', 'Column'])
        self.queue_columns_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Fixed)
        self.queue_columns_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.queue_columns_table.setColumnWidth(0, 60)
        self.queue_columns_table.verticalHeader().setVisible(False)
        self.queue_columns_table.setSelectionMode(QAbstractItemView.NoSelection)
        self.queue_columns_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.queue_columns_table.setFixedHeight(240)
        self._reload_queue_columns_table()
        parent_layout.addWidget(self.queue_columns_table)

    def _reload_queue_columns_table(self):
        cols = self._settings.get('queue_columns') or []
        always_on = {'jobid', 'state'}
        self.queue_columns_table.setRowCount(len(cols))
        for i, col in enumerate(cols):
            key = str(col.get('key') or '')
            label = str(col.get('label') or key)
            cb_holder = QWidget()
            cb_lay = QHBoxLayout(cb_holder)
            cb_lay.setContentsMargins(4, 0, 0, 0)
            cb = QCheckBox()
            cb.setChecked(bool(col.get('enabled')) or key in always_on)
            if key in always_on:
                cb.setEnabled(False)
            cb_lay.addWidget(cb)
            cb_lay.addStretch()
            self.queue_columns_table.setCellWidget(i, 0, cb_holder)
            label_item = QTableWidgetItem(f'{label}   ({key})')
            self.queue_columns_table.setItem(i, 1, label_item)

    def _collect_queue_columns(self):
        cols = list(self._settings.get('queue_columns') or [])
        for i, col in enumerate(cols):
            holder = self.queue_columns_table.cellWidget(i, 0)
            if holder is None:
                continue
            cb = holder.findChild(QCheckBox)
            if cb is not None:
                col['enabled'] = bool(cb.isChecked())
        return cols
    ...
    def _move_ssh_button(self, direction):
        """direction: -1 = up, +1 = down. Swap selected row with its neighbor."""
        table = self.ssh_buttons_table
        ...
        if target < 0 or target >= table.rowCount():
            return
        # Collect both rows' data, then rewrite them swapped.
        def snapshot(r):
            use = table.item(r, 0)
            return (
                (use.checkState() == Qt.Checked) if use else True,
        ...
        b = snapshot(target)
        def write(r, data):
            enabled, label, cmd = data
            use_item = QTableWidgetItem('')
            use_item.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled | Qt.ItemIsSelectable)
            use_item.setCheckState(Qt.Checked if enabled else Qt.Unchecked)
            table.setItem(r, 0, use_item)
            table.setItem(r, 1, QTableWidgetItem(label))
            table.setItem(r, 2, QTableWidgetItem(cmd))
        write(row, b)
        write(target, a)
        table.setCurrentCell(target, table.currentColumn() if table.currentColumn() >= 0 else 1)
    ...
    def _connect_signals(self):
        ...
        self.ssh_button_up_btn.clicked.connect(lambda: self._move_ssh_button(-1))
        self.ssh_button_down_btn.clicked.connect(lambda: self._move_ssh_button(+1))
    ...
    def _snapshot_settings(self, capture=False):
        ...
        s.update({
            ...
            'bjobs_command': self.bjobs_command_input.text().strip() or 'bjobs -u {user}',
            'queue_columns': self._collect_queue_columns(),
            'queue_detail_on_select': bool(self.queue_detail_on_select_cb.isChecked()),
        })
