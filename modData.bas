...
' Replaces the FILES list with the selected CSV files.
...
Private Sub ParseFileName(ByVal filePath As String, _
    ...
    Dim baseName As String, rpmText As String, nameParts() As String
    ...
    nameParts = Split(baseName, "_")
    If UBound(nameParts) < 3 Then
        Err.Raise vbObjectError + 104, "ParseFileName", _
                  "Invalid CSV file name: " & baseName
    End If

    modelName = Trim$(nameParts(0))
    bandName = UCase$(Trim$(nameParts(1)))

    If bandName <> "NB" And bandName <> "OB" Then
        Err.Raise vbObjectError + 105, "ParseFileName", _
                  "Band must be NB or OB: " & baseName
    End If

    rpmText = nameParts(UBound(nameParts))
    ...
    loadName = ""
    For partIndex = 2 To UBound(nameParts) - 1
        If Len(loadName) > 0 Then loadName = loadName & "_"
        loadName = loadName & nameParts(partIndex)
    Next partIndex
