...
Public Sub ImportAll(Optional ByVal silent As Boolean = False)
    ...
    Dim stage As String, summary As String
    ...
    If mImportedFiles > 0 Then
        stage = "fill CALC_NB": FillNarrowbandFormulas
        stage = "fill CALC_OB": FillOctavebandFormulas
        stage = "calculate CALC_NB"
        ThisWorkbook.Worksheets("CALC_NB").Calculate
        stage = "calculate CALC_OB"
        ThisWorkbook.Worksheets("CALC_OB").Calculate
        stage = "refresh Model slots": RefreshModelSlots
        stage = "refresh SPL": modSPL.RefreshSPL
        stage = "refresh OVERALL": modOverall.RefreshOverall
    Else
...
finished:
    ...
    If Not silent Then MsgBox summary, _
        IIf(errorNumber = 0, vbInformation, vbExclamation), "Import data"
